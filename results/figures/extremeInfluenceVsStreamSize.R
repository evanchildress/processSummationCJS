...
tiff.par("results/figures/extremeInfluenceVsStreamSize.tif",mfrow=c(1,2),
         mar=c(2.5,3,0,0),width=6.5,height=4)

  plot(extremeInfluence~log(medianQ),data=extInf[stage==1],ylim=c(0,0.5),
       xlab="log(Median Discharge)",ylab="",pch=19,col=gray(0.5,0.5))
  title(ylab="Proportion Events Accounting for 50% Mortality",line=2.2)
  plot(extremeInfluence~log(medianQ),data=extInf[stage==2],ylim=c(0,0.5),
       xlab="log(Median Discharge)",ylab="",pch=19,col=gray(0.5,0.5))
  #title(ylab="Proportion Events Accounting for 50% Mortality",line=2.2)
