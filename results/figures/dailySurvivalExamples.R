...
par(mfrow=c(3,4))
for(y in 2004:2015){
plot(westBrookYoy~date,data=surv[year(date)==y],type='l',col=palette()[1],ylim=c(0.95,1))
...
plot(westBrookYoy~date,data=surv[year(date)==2010&month(date) %in% c(7,8,9)],
     type='l',col=palette()[1],ylim=c(0.95,1),xlab="",ylab="")
points(jimmyYoy~date,data=surv[year(date)==2010&month(date) %in% c(7,8,9)],type='l',col=palette()[2])
points(mitchellYoy~date,data=surv[year(date)==2010&month(date) %in% c(7,8,9)],type='l',col=palette()[3])
points(obearYoy~date,data=surv[year(date)==2010&month(date) %in% c(7,8,9)],type='l',col=palette()[4])
...
for(r in 1:4){
  temps<-tempData[as.Date(rownames(tempData)) %in% seq.Date(as.Date("2010-07-01"),as.Date("2010-09-30"),by="day"),r]*
    stds$temp$sdTemp[r]+stds$temp$meanTemp[r]
  points(temps~seq.POSIXt(as.POSIXct("2010-07-01"),as.POSIXct("2010-09-30"),by="day"),type='l',col=palette()[r])
}
par(new=T)
...
mtext("Scaled log(Discharge)",4,line=1.5,las=0,cex=0.666)
for(r in 1:4){
  flows<-flowData[as.Date(rownames(flowData)) %in% seq.Date(as.Date("2010-07-01"),as.Date("2010-09-30"),by="day"),r]
  points(flows~seq.POSIXt(as.POSIXct("2010-07-01"),as.POSIXct("2010-09-30"),by="day"),
         type='l',col=palette()[r],lty=2)
}
